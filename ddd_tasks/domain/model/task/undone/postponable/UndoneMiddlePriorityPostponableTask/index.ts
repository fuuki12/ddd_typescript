...
export class UndoneMiddlePriorityPostponableTask implements UndoneTask {
  ...
  static create(id: number, name: string, postponeCount: number) {
    return new UndoneMiddlePriorityPostponableTask(
      id,
      name,
      "middle",
      postponeCount,
  ...
  public get done(): DoneMiddlePriorityTask {
    return DoneMiddlePriorityTask.create(
      this._id,
      this._name,
      this._postponeCount
    );
