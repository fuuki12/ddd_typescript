...
export class UndoneLowPriorityPostponableTask implements UndoneTask {
  ...
  static create(id: number, name: string, postponeCount: number) {
    return new UndoneLowPriorityPostponableTask(
      id,
      name,
      "low",
      postponeCount,
  ...
  public get done(): DoneLowerPriorityTask {
    return DoneLowerPriorityTask.create(
      this._id,
      this._name,
      this._postponeCount
    );
